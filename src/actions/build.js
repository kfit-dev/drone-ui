...
/**
 * restartBuild swapns the a new build from an existing entry
 * and dispatches an event to add the object to the store.
 */
export const restartBuild = async ({ commit }, { namespace, name, build }) => {
...
/**
 * createBuild creates a new custom build
 */
export const createBuild = async ({ commit }, { namespace, name, branch, commit:commitSha, params }) => {
  commit(BUILD_CREATE_LOADING);

  const query = queryString({branch, commit: commitSha, ...params});

  const req = await fetch(`${instance}/api/repos/${namespace}/${name}/builds?${query}`, {
    headers,
    method: "POST",
    credentials: "same-origin"
  });
...
export const createDeployment = async ({ commit }, { namespace, name, build, target, action, params }) => {
  ...
  const query = queryString({target, ...params});

  const req = await fetch(`${instance}/api/repos/${namespace}/${name}/builds/${build}/${action}?${query}`, {
    headers,
    method: "POST",
    credentials: "same-origin"
  });
